...
export function download(
  ...
  const filename =
    Multilingualization.translate('app_name') +
    '_' +
    getTodayString() +
    '.' +
    extension;
...
export function parse(text, mins) {
  ...
  Object.keys(obj).forEach((item) => {
    obj[item].round =
      Math.floor(obj[item].time / 60) +
      Number(
        ((Math.round((obj[item].time % 60) / mins) * mins) / 60).toFixed(2),
      );
  });
...
<td class='text-end'>${dataJson[category].time}</td>
  ...
  const sumStr =
    Multilingualization.translate('work_time_actual') +
    '： ' +
    (Math.floor(sum / 60) +
      Number(((Math.round((sum % 60) / mins) * mins) / 60).toFixed(2))) +
    ' h';
  const totalStr =
    Multilingualization.translate('work_time_total') +
    '： ' +
    (Math.floor(total / 60) +
...
export function toMarkdown(log, mins) {
  ...
  let sum = 0;
  let total = 0;
  let output = `${Multilingualization.translate('work_category')} | ${Multilingualization.translate('work_detail')} | ${Multilingualization.translate('work_time_hour')} | ${Multilingualization.translate('work_time_min')}
--- | --- | --: | --:
`;
  ...
  output +=
    ...
    (Math.floor(sum / 60) +
      Number(((Math.round((sum % 60) / mins) * mins) / 60).toFixed(2))) +
    ` h (${sum} ${Multilingualization.translate('mins')})`;
  output +=
    `\n${Multilingualization.translate('work_time_total')}： ` +
    (Math.floor(total / 60) +
      Number(((Math.round((total % 60) / mins) * mins) / 60).toFixed(2))) +
    ` h (${total} ${Multilingualization.translate('mins')})`;
