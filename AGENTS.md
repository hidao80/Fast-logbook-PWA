...
Playwright tests live in `tests/e2e/` and run against `http://localhost:3000`. The config auto-starts `bun run dev` when not in CI.
...
- CI runs `bun run lint` and `bun audit` on push/PR to `main`/`develop`.
