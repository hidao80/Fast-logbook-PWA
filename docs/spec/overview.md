...
- **Bootstrap 5.3.8** + **Bootstrap Icons 1.13.1** — installed as npm packages and imported in `main.tsx` (no longer CDN-loaded for the app shell)
- **idb** — a Promise-based IndexedDB wrapper used for data persistence

### Build & Development
- **Vite 8** (`@vitejs/plugin-react`) — dev server (`npm run dev`, port 3000) and production build (`npm run build` → `dist/`)
...
- **Package Manager**: npm (primary; `package-lock.json` is committed). A `pnpm-lock.yaml` also exists locally but is gitignored.

### CI/CD
- **GitHub Actions**: `lint.yml` (Biome via `reviewdog-action-biome`) and `audit.yml` (`npm audit --audit-level=high` + Takumi Guard supply-chain scan, pnpm-based)

### PWA Features
- **Service Worker**: Generated by `vite-plugin-pwa`/Workbox; `registerType: 'autoUpdate'`, `devOptions.enabled: true` so it also runs under `npm run dev`
...
- **Netlify** (current deployment target): `netlify.toml` runs `npm run build` and publishes `dist/`
- **Docker**: multi-stage build (`node:24-bookworm-slim` → `nginx:alpine`), `npm ci` + `npm run build`, served by Nginx on port 80
